...
#if defined(NDEBUG)
# error "Bokkcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bokkcoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetBokkcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Bokkcoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("bokkcoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Bokkcoin: BIP30 has been active since inception
    ...
    // Bokkcoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetBokkcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Bokkcoin: Disallow AuxPow blocks before it is activated.
    ...
    // Bokkcoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Bokkcoin: We probably want to disable this
    int nLockTimeFlags = 0;
