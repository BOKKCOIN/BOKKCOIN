...
// Copyright (c) 2021 BOKKCOIN Core developer(s)
...
#ifdef ENABLE_WALLET
//mlumin 5/2021: walletfees, all attached to GetBokkcoinWalletFeeRate which is just the newly exposed ::minWalletTxFee
CAmount GetBokkcoinWalletFee(size_t nBytes_)
{
    //mlumin: super simple fee calc for bokkcoin
    CAmount nFee=GetBokkcoinWalletFeeRate().GetFee(nBytes_);
}

//mlumin 2021: Establish a wallet rate of n koinu per kb.
...
{
    //mlumin 021: currently 1x COIN or 1 bokkcoin or 100,000,000 koinu
...
CAmount GetBokkcoinMinRelayFee(const CTransaction& tx, unsigned int nBytes, bool fAllowFree)
{
    ...
    nMinFee += GetBokkcoinDustFee(tx.vout, ::minRelayTxFeeRate);
