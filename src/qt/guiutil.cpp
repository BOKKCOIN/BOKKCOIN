...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Bokkcoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no bokkcoin: URI
    if(!uri.isValid() || uri.scheme() != QString("bokkcoin"))
        return false;
...
{
    // Convert bokkcoin:// to bokkcoin:
    //
    //    Cannot handle this later, because bokkcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bokkcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "bokkcoin:");
    }
...
{
    QString ret = QString("bokkcoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bokkcoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Bokkcoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Bokkcoin (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Bokkcoin\n";
        else
            optionFile << strprintf("Name=Bokkcoin (%s)\n", chain);
