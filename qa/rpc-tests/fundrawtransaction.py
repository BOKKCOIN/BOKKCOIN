...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']}]
        # Bokkcoin: Fee is exact, do not use tolerance
        ...
        assert_raises_jsonrpc(-5, "changeAddress must be a valid bokkcoin address", self.nodes[2].fundrawtransaction, rawtx, {'changeAddress':'foobar'})
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']} ]
        # Bokkcoin: Reduce this output so the fee doesn't leave us with no change
        ...
        inputs = []
        # Bokkcoin: TX size rounding gives us a fee of 1 BOKK. 20 - 15 - 1 = 4 BOKK change
